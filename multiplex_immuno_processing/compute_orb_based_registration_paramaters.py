...
if __name__ == "__main__":
    ...
    for i in range(len(img_list)):
        try:
            ...
            source_img_FOV_cropped = new_source_img[:, 100:np.shape(new_source_img)[1]-100, 100:np.shape(new_source_img)[2]-100]
            ...
            alignment_offsets_xyz_list_new_version.append((final_x_offset, final_y_offset))
        except:
            alignment_offsets_xyz_list_new_version.append((0,0))

            print(f"not fitted for {i}")
    #print("for first img in new_source_img, z_offset is", final_z_offset)


    print(f"alignment offset xyz old version is {alignment_offsets_xyz_list}")
    print(f"alignment offset xyz new version is {alignment_offsets_xyz_list_new_version}")


    final_version_offsets_zyx=[]
    # This just copies over the z offset from the cross correlation method
    for k in range(len(alignment_offsets_xyz_list_new_version)):
        dat=alignment_offsets_xyz_list_new_version[k]
        final_version_offsets_zyx.append([alignment_offsets_xyz_list[k][0], 100-dat[0], 100-dat[1]])
    print(f"final version offset xyz is {final_version_offsets_zyx}")
    print("outputing")



    dfimg["alignment_offsets_xyz"] = final_version_offsets_zyx
    ...
    output_dir = dfconfig["output_path"][0]
    pickle_dir = output_dir + os.sep + "alignment_pickles_each_ORB_method"
