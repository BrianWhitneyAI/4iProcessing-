...
if __name__ == "__main__":
    ...
    for bdir in args.input_dirs:  # for each input directory
        ...
        #subfolders = [f for f in os.listdir(bdir) if f.is_dir()]
        # import pdb
        # pdb.set_trace()

        round_list = [x for x in os.listdir(bdir) if bool(re.search("Time|R[0-9]+", x, re.IGNORECASE)) & (Path(x).stem == Path(x).name)] # gets list of rounds and timelapse
        print(f"round list is {round_list}")
        #round_list = sort_rounds(round_list)
        print("round list is {}".format(round_list))
        for round_num in round_list:
            ppath = os.path.join(bdir, round_num)
            Image_list = [
                x
