...
def remove_overlapping_lowmag(dfmag1, adjust=False):
    # this block here will assigns each mag2 image position to
    # mag1 image position based on imaging region overlap determind from metadata
    # this block here will exclude positions for analysis if the regions of imaging overlapped
    # (i.e. it identifies multiple mag1 positions for a given mag2 position)
    ...
    for i in range(dfmag1.shape[0]):
        ...
        xyz = (dfmag1.iloc[i].at["X"], dfmag1.iloc[i].at["Y"], dfmag1.iloc[i].at["Z"])
        ...
        for u in range(dfmag1.shape[0]):
            ...
            for k in range(2):
                loglist.append(
                    (xyz[k] > (xyz2[k] - imgsize_um_mag1[k]))
                    & (xyz[k] < (xyz2[k] + imgsize_um_mag1[k]))
                )
            if i != u:
        ...
        if len(pnum) != 0:
            ...
            # TODO: make this based on imaging time, for now you should just remove all overlaps
    ...
    if np.sum(~keep_bool) > 0:
        print(
            "tossing because of self-overlap: \n",
...
def find_matching_positions_and_remove_duplicates_and_incompletes(
    dftemp, dfmove, drop_incompletes=True
):

    img_size_temp = dftemp["imgsize_um"].iloc[0]

    # now you need to find all "dfmove" cooridantes, that is the higher mag dataframe that could have more positons
    # tiled onto a lower mag image that are closer together than 1.2 the img_size of the dftemp
    keeplist = []
    ...
    for xyz in ["X", "Y", "Z"]:
        temp[xyz] = (
            temp[xyz] - temp.iloc[0].at[xyz]
        )  # normalize all positions to first position of template
    ...
    for refnum in range(dftemp.shape[0]):
        ...
        lessthan_values = cmat[lessthan_idx].reshape(
            -1,
        )
    ...
    if (
        np.sum(ltmin_idx) > 1
    ):  # write code to choose the one with the least distance difference
    ...
    for i in range(move.shape[0]):
        print(i, end="\r")
        xyz = np.asarray(
            (move.iloc[i].at["X"], move.iloc[i].at["Y"], move.iloc[i].at["Z"])
        )

        contained_log = np.bool_(np.zeros(temp.shape[0]))
        for u in range(temp.shape[0]):
            xyz2 = np.asarray(
                (temp.iloc[u].at["X"], temp.iloc[u].at["Y"], temp.iloc[u].at["Z"])
            )
        ...
        if len(pnum) > 1:
            ...
            print(
                "no match found for scene =",
                dfmove.iloc[i].at["Scene"],
                " , ",
                dfmove.iloc[i].at["Position"],
                " , ",
                dfmove.iloc[i].at["fname"],
            )
            plist.append("")
            elist.append(True)
        else:
    ...
    print()
    print(
        "dropping from dfmove (no match):",
        dfmove[dfmove["elist"] is True]["Position"].unique(),
    )
    dfmove = dfmove[dfmove["elist"] is False]  # only keep non duplicates, and matches

    if drop_incompletes:
        dfcount = dfmove.groupby(
            ["mag1_position_number"]
        ).count()  # count to determine number of matching high mag tiles per low mag image
        drop_incomplete_list = dfcount.index.values[
            dfcount["file"] < dfcount["file"].max()
        ]  # determine to drop everything that does not have complete tiled matches
        complete_bool = [
            True if x not in drop_incomplete_list else False
            for x in dfmove["mag1_position_number"].tolist()
        ]
        print(
            "dropping from dfmove",
            dfmove[~np.asarray(complete_bool)]["mag1_position_number"].unique(),
        )
    ...
    keeplist = [
        True if x in mag1list else False
        for x in dftemp["mag1_position_number"].tolist()
    ]
    print(
        "dropping from mag1",
        dftemp[~np.asarray(keeplist)]["mag1_position_number"].unique(),
    )
...
def remove_duplicates_and_incompletes(dfmag1, dfmag2, drop_incompletes=True):
    # this block here will assigns each mag2 image position to a mag1 image position based on
    # imaging region overlap determind from metadata.
    ...
    for i in range(dfmag2.shape[0]):
        print(i, end="\r")
        xyz = (dfmag2.iloc[i].at["X"], dfmag2.iloc[i].at["Y"], dfmag2.iloc[i].at["Z"])
        ...
        for u in range(dfmag1.shape[0]):
            ...
            for k in range(2):
                loglist.append(
                    (xyz[k] > (xyz2[k] - imgsize_um_mag1[k] / 2))
                    & (xyz[k] < (xyz2[k] + imgsize_um_mag1[k] / 2))
                )
            contained_log[u] = loglist[0] & loglist[1]
        ...
        if len(pnum) > 1:
            ...
            print(
                "no match found for scene =",
                dfmag2.iloc[i].at["Scene"],
                " , ",
                dfmag2.iloc[i].at["Position"],
                " , ",
                dfmag2.iloc[i].at["fname"],
            )
            plist.append("")
            elist.append(True)
        else:
    ...
    print()
    print(
        "dropping from mag2 (no match):",
        dfmag2[dfmag2["elist"] is True]["Position"].unique(),
    )
    dfmag2 = dfmag2[dfmag2["elist"] is False]  # only keep non duplicates, and matches

    if drop_incompletes:
        dfcount = dfmag2.groupby(
            ["mag1_position_number"]
        ).count()  # count to determine number of matching high mag tiles per low mag image
        drop_incomplete_list = dfcount.index.values[
            dfcount["file"] < dfcount["file"].max()
        ...
        complete_bool = [
            True if x not in drop_incomplete_list else False
            for x in dfmag2["mag1_position_number"].tolist()
        ]
        print(
            "dropping from mag2",
            dfmag2[~np.asarray(complete_bool)]["mag1_position_number"].unique(),
        )
    ...
    keeplist = [
        True if x in mag1list else False
        for x in dfmag1["mag1_position_number"].tolist()
    ]
    print(
        "dropping from mag1",
        dfmag1[~np.asarray(keeplist)]["mag1_position_number"].unique(),
    )
...
def toss_scenes(df, scenes_to_toss_dict):
    ...
    for fname, scenelist in scenes_to_toss_dict.items():
        ...
        log = (np.asarray(df["fname"] == fname)) & (
            np.asarray(df["Scene"].apply(lambda x: int(x) in scenelist))
        )
...
def toss_scenes_old(df, scenes_to_toss_dict):
    ...
    dfout = dfkeeps[dfkeeps["toss_bool"] is False]
    print("tossing scenes = ", dfkeeps[dfkeeps["toss_bool"] is True]["Scene"].tolist())
    print(
        "tossing positions = ",
        dfkeeps[dfkeeps["toss_bool"] is True]["Position"].tolist(),
    )
    ...
    for regions in meta.findall(".//Scenes"):
        for region in regions.findall(".//Scene"):
            ...
            for info in region.findall(".//Position"):
                ...
                feats["X"] = float(
                    info.attrib["Y"]
                )  # need to switch Y and X to match ZSD coordinates
...
def make_new_positions_in_tiles(
    df_orig, mag=100, num_updown=3, num_leftright=3, lrgap=20, udgap=20
):
    ...
    id_listnew = unique_id_list_generator(
        df_orig, offset_list
    )  # return list of unique ids for czsh file type
    keeplist = []
    cycle = -1
...
def write_dataframe_into_xmlold(filename, df):
    ...
    pad_list = [
        x
        for x in pad_list
        if bool(re.search("Tileregions|Tileregionarrays", x, re.IGNORECASE))
    ]
...
def write_zen_positionfile_fromczi(
    old_filename, xml_tree, xml_root, mag1, mag2, extrastr=""
):
...
def plot_matched_positions(dfmatch, suffixes=("_mag1", "_mag2")):
    ...
    dfmatch["mag1_position_number" + suffixes[0]] = dfmatch[
        "Position" + suffixes[0]
    ...
    for well, dfm in dfmatch.groupby("Well_id" + suffixes[0]):
        ...
        for mag1pos, dfsub in dfm.groupby(["mag1_position_number" + suffixes[0]]):
            ...
            w = imgsize2[0]
            h = imgsize2[1]
            for _, (mag2pos, dfsubsub) in enumerate(
                dfsub.groupby(["Position" + suffixes[1]])
            ):
                try:
...
def plot_all_positions(dfmag1, dfmag2, adjust=False):
    ...
    fig, axr = plt.subplots(
        nrows=len(well_list), ncols=1, figsize=(10, 10 * len(well_list))
    )
    try:
        axlist = axr.reshape(
            -1,
    ...
    for di, (df1, cc) in enumerate(zip([dfmag1, dfmag2], ["y", "r"])):
        ...
        for wi, (well, dfm) in enumerate(df1.groupby("Well_id")):
            plt.sca(axlist[wi])
            plt.title(well)
            for _, dfsub in dfm.groupby(["mag1_position_number"]):
...
def generate_match(dfmag1, dfmag2, suffixes=("_mag1", "_mag2")):
    ...
    #     dfmatch = dfmatch[dfmatch['IsUsedForAcquisition'+suffixes[1]]is'true']
    #     dfmatch = dfmatch[dfmatch['IsUsedForAcquisition'+suffixes[0]]is'true']
    dfmatch = dfmatch[dfmatch["elist"] is False]
    ...
    chosen_columns = [
        ...
        "laser_intensity",
        "channel_dict",
        suffixes[0].replace("_", "") + "_position_number" + suffixes[0],
    ]
    choose_columns = [
        x for x in columns if bool(re.search("|".join(chosen_columns), x))
    ]
    ...
    for i in range(dfmatch.shape[0]):
        xyz20 = tuple(
            list(
                dfmatch[["X" + suffixes[0], "Y" + suffixes[0], "Z" + suffixes[0]]].iloc[
                    i
                ]
            )
        )
        xyz100 = tuple(
            list(
                dfmatch[["X" + suffixes[1], "Y" + suffixes[1], "Z" + suffixes[1]]].iloc[
                    i
                ]
            )
        )
    ...
    try:
        dfmatch.insert(
            2,
            column=suffixes[1].replace("_", "")
            + "_xy_offset_from"
            + suffixes[0]
            + "_center",
            value=xy_offset_from_mag1_center,
...
def unique_id_list_generator(df_twentyX, sl):
    ...
    while li < cycle_final:
        id_listnew.append(
            "6371395" + "".join(random.choice(string.digits) for _ in range(N))
        )
