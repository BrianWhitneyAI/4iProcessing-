...
if __name__ == "__main__":
    ...
    for filepath in globout:
        ...
        dfm = dfm.sort_index(level=dfm.index.names) #this is needed to handle warning that arises in pandas

        print(dfm.loc[ivs,['overlap','move_position','Scene','overlapping_positions_within_same_round']+flagcols])
        print()
        ...
        print('number of positions imaged a different number of times than the number of rounds=', dfgp_diffnumber.shape[0])
        print('positions that were imaged more than once per round',dfgp[dfgp['file']>number_of_rounds].index.values)
        print('positions that were imaged less than once per round',dfgp[dfgp['file']<number_of_rounds].index.values)
        ...
        dfgkp['not_present_in_these_rounds'] = dfgkp['Position'].apply(lambda x: [y for y in positions if y not in x])
        dfgkp['present_more_than_once_in_these_rounds'] = dfgkp['Position'].apply(lambda x: [y for y in positions if np.sum([True for xx in x if xx==y])>1])
            
        
        print(dfgkp[['not_present_in_these_rounds','present_more_than_once_in_these_rounds']])
        print()