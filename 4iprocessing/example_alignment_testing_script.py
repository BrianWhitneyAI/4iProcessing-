...
points =np.array([[  10.        ,  880.4721025 , 1124.09191905],
       [  14.        ,  884.03271896, 1061.42506935]])
...
for i,(point, img) in enumerate(zip(points,max_imglist)):
    ...
    img_slice_rs =skex.rescale_intensity(img_slice, in_range=(lp,hp),out_range='uint8').astype('uint8')
    color=colorlist[i]
    if color=='green':
        color_vec = [1]
    else:
...
for i,roundname in enumerate(roundlist):
    ...
    plt.text(0,80*i, roundname, fontsize=12,color=colorlist[i],
            va='top',)
plt.axis('off')
...
align_shift_list = [np.asarray([0,0,0]),
                   align_shift]
...
for alignment_offset,img in zip(align_shift_list,imglist):
    ...
    #aligned image
    processed_volume = affine_transform(img, 
                                        np.linalg.inv(combo),
    ...
    #unaligned image
    center_processed_volume = affine_transform(img, 
...
for pi, (processed_img_list,aligned_or_unaligned) in enumerate(zip([aligned_img_list,unaligned_img_list],
                                                  ['aligned','unaligned'])):
    plt.sca(axlist[pi])
...
# compare alignment with dfalign value
try:
    output_dir = dfconfig['output_path'][0]
